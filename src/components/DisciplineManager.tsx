...
export function DisciplineManager({
  ...
  return <>
      <motion.div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4" variants={containerVariants} initial="hidden" animate="show">
        {disciplines.map(discipline => <motion.div key={discipline} className="bg-clay-blue p-4 rounded-xl shadow-clay hover:shadow-clay-pressed transition-all relative group" variants={itemVariants} whileHover={{
        scale: 1.03
      }} whileTap={{
        scale: 0.98
      }}>
            ...
            <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity flex gap-1">
              <Button variant="ghost" size="icon" className="h-6 w-6" onClick={() => handleEdit(discipline)}>
                <PencilIcon size={14} />
              </Button>
              <Button variant="ghost" size="icon" className="h-6 w-6 text-destructive" onClick={() => handleDelete(discipline)}>
                <Trash2Icon size={14} />
              </Button>
            </div>
          </motion.div>)}
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div className="py-4">
            <Input value={newDisciplineName} onChange={e => setNewDisciplineName(e.target.value)} placeholder="Nome da disciplina" className="clay-input" />
          </div>
          <DialogFooter>
