...
const Index = ({
  ...
  return <Layout disciplines={disciplines} onAddDiscipline={onAddDiscipline} onEditDiscipline={onEditDiscipline} onDeleteDiscipline={onDeleteDiscipline} onSearch={setSearchTerm}>
      ...
      <motion.div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-6 mb-6" variants={containerVariants} initial="hidden" animate="visible">
        <motion.div variants={itemVariants}>
          <StatusCard title="Tarefas Pendentes" value={pendingTasksCount} icon={<CircleIcon size={24} className="text-yellow-600" />} color="bg-clay-yellow" subtitle={`${Math.round(pendingTasksCount / Math.max(totalTasks, 1) * 100)}% do total`} />
        </motion.div>
        ...
        <motion.div variants={itemVariants}>
          <StatusCard title="Concluídas" value={completedTasksCount} icon={<CircleCheckIcon size={24} className="text-green-600" />} color="bg-clay-mint" subtitle={`${completedPercentage}% concluído`} />
        </motion.div>
        
        <motion.div variants={itemVariants}>
          <StatusCard title="Disciplinas" value={totalDisciplines} icon={<BookOpenIcon size={24} className="text-blue-600" />} color="bg-clay-blue" subtitle="Total de disciplinas" />
        </motion.div>
      </motion.div>
      
      {/* Quick Links */}
      <motion.div className="clay-card mb-6 bg-gradient-to-r from-primary/5 to-accent/5" initial={{
      opacity: 0,
      y: 20
    }} animate={{
      opacity: 1,
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        ...
        delay: 0.2
          ...
          <div className="space-y-4 max-h-60 md:max-h-80 overflow-y-auto pr-2">
            {pendingTasks.length > 0 ? pendingTasks.map(task => <TaskCard key={task.id} task={task} onStatusChange={handleStatusChange} onEdit={handleEditTask} onDelete={handleDeleteTask} disciplines={disciplines} />) : <div className="text-center p-4 bg-gray-50 rounded-lg">
      ...
      <motion.h2 className="text-xl md:text-2xl font-bold mb-4" initial={{
      opacity: 0,
      x: -20
    }} animate={{
      opacity: 1,
      ...
      <motion.div className="clay-card mb-8" variants={containerVariants} initial="hidden" animate="visible">
        <div className="space-y-4">
          {filteredTasks.length > 0 ? filteredTasks.map((task, index) => <motion.div key={task.id} variants={itemVariants} initial="hidden" animate="visible" transition={{
          delay: 0.1 * index
        }} whileHover={{
          scale: 1.01
        }}>
                <TaskCard task={task} onStatusChange={handleStatusChange} onEdit={handleEditTask} onDelete={handleDeleteTask} disciplines={disciplines} />
              </motion.div>) : <motion.div className="flex flex-col items-center p-8" variants={itemVariants}>
              ...
              {!searchTerm && <motion.div whileHover={{
            scale: 1.05
          }} whileTap={{
            scale: 0.95
          }}>
      ...
      <motion.h2 className="text-xl md:text-2xl font-bold mt-8 mb-4" initial={{
      opacity: 0,
      x: -20
    }} animate={{
      opacity: 1,
      ...
      delay: 0.5
        ...
        <DisciplineManager disciplines={disciplines} onAddDiscipline={onAddDiscipline} onEditDiscipline={onEditDiscipline} onDeleteDiscipline={onDeleteDiscipline} />
      </motion.div>
    </Layout>;
};
